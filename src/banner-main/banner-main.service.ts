...
@Injectable()
export class BannerMainService {
  constructor(
    private readonly databaseService: DatabaseService,
    private uploadPhotoService: UploadPhotoService,
  ) {}

  async create(createBannerMainDto: CreateBannerMainDto): Promise<{
    success: boolean;
    message: string;
  }> {
    let result = null;
    ...
    if (result && result.rows.length > 0) {
      throw new ConflictException(
        'Ошибка при создании баннера: похожее название уже существует',
      );
    }
  ...
  async findAll() {
    try {
      const result = await this.databaseService.executeOperation({
        operation: GRUD_OPERATION.QUERY,
        query:
          'SELECT id::int,seconds,url, type FROM banner_main ORDER BY id DESC',
        params: [],
      });
      return {
        success: true,
  ...
  async findOne(id: number) {
    try {
      const result = await this.databaseService.executeOperation({
        operation: GRUD_OPERATION.QUERY,
        query:
          'SELECT id::int,seconds,url, type FROM banner_main WHERE id = $1',
        params: [id],
      });
  ...
  async update(id: number, updateBannerMainDto: UpdateBannerMainDto) {
    ...
    try {
      ...
      if (updateBannerMainDto.name) {
        const NameBanner = transformName(updateBannerMainDto.name);
        ...
        if (result && result.rows.length > 0) {
          await this.databaseService.rollbackTransaction(transaction);
          throw new ConflictException(
            'Баннер стартового экрана с таким названием уже существует',
          );
        }

        updateBannerMainDto.name = NameBanner;
      }
      const { file, ...updateData } = updateBannerMainDto;
      ...
      return {
        success: true,
        data: result,
        message: 'Баннер стартового экрана успешно обновлен',
      };
    } catch (error: any) {
      await this.databaseService.rollbackTransaction(transaction);
      throw new BadGatewayException(error.message);
  ...
  async remove(id: number) {
    try {
      ...
      return {
        success: true,
        message: 'Баннер стартового экрана успешно удален',
      };
    } catch (error: any) {
      return {
        success: false,
        message: `Ошибка при удалении баннера стартового экрана: ${error.message}`,
      };
    }
  }
}
