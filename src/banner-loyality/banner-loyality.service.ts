    ...
    try {
      const result = await this.databaseService.executeOperation({
        ...
        query: `SELECT id:: int, seconds:: int, url, type, name, type, store, is_active as
           "isActive", "create_at" as "createAt", "updated_at" as "updatedAt" FROM banner_loyality ORDER BY id DESC`,
        params: [],
      });
      return {
        success: true,
  ...
  async findOne(id: number) {
    try {
      const result = await this.databaseService.executeOperation({
        operation: GRUD_OPERATION.QUERY,
        query: `SELECT id:: int, seconds:: int, url, type, name, type, store, is_active as
           "isActive", "create_at" as "createAt", "updated_at" as "updatedAt" FROM banner_loyality WHERE id = $1`,
        params: [id],
      });
